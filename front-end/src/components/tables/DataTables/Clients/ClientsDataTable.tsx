...
import ClientDropdownFilter, { ClientFilters } from "@/components/example/DropdownExample/ClientDropdownFilter";

type SortKey = "id" | "name" | "email" | "phoneNumber" | "status";
...
export default function ClientsDataTable({clients, onClientAdded}: ClientsDataTableProps) {
  ...
  const filteredData = useMemo(() => {
    ...
    if (filters.client || filters.lead) {
      filtered = filtered.filter((item) => {
        const status = item.status.toUpperCase();
        if (filters.client && status === "CLIENT") return true;
        if (filters.lead && status === "LEAD") return true;
        return false;
      });
    }

    return filtered;
  }, [clientsData, searchTerm, filters]);
