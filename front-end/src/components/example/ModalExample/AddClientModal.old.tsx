...
export default function AddClientModal({ onClientAdded }: AddProjectModalProps) {
  ...
  const handleSave = async () => {
    ...
    console.log("📝 [Modal] Current form data:", formData);
    console.log("🏠 [Modal] Selected apartments:", selectedApartments);
    ...
    console.log("🏠 [Modal] Flattened apartment IDs:", allApartmentIds);

    const formDataToSend = new FormData()

    // Combine firstName and lastName into name for backend
    const fullName = `${formData.firstName.trim()} ${formData.lastName.trim()}`.trim()
    console.log("👤 [Modal] Combined full name:", fullName);
    formDataToSend.append("name", fullName)

    // Add all form fields except apartmentId, firstName, lastName (already combined into name)
    console.log("📋 [Modal] Adding form fields to FormData:");
    Object.entries(formData).forEach(([key, value]) => {
      if (key !== "apartmentId" && key !== "firstName" && key !== "lastName" && key !== "confirmPassword") {
    ...
    console.log("🏠 [Modal] Adding apartment IDs:");
    ...
    if (identityRecto) {
      console.log("📄 [Modal] Adding identity recto file:", identityRecto.name, identityRecto.size, "bytes");
      formDataToSend.append("identityRecto", identityRecto)
    }
    if (identityVerso) {
      console.log("📄 [Modal] Adding identity verso file:", identityVerso.name, identityVerso.size, "bytes");
      formDataToSend.append("identityVerso", identityVerso)
    }

    try {
      console.log("🚀 [Modal] Sending form data to API...");
      const result = await addClient(formDataToSend)
      console.log("✅ [Modal] Client creation successful:", result);
      ...
      console.log("🎉 [Modal] Form reset and modal closing");
      closeModal()
    } catch (error) {
      console.error("💥 [Modal] Error adding client:", error);
  ...
  const validateForm = (): boolean => {
    ...
    // Validate password for CLIENT status
    ...
    if (formData.status === "CLIENT") {
      if (!formData.identityType.trim()) {
        newErrors.identityType = "Le type de pièce d'identité est requis";
        valid = false;
      }
              ...
              {formData.status === "CLIENT" && (
                ...
                <div className="col-span-1">
                  ...
                  <Select
                    ...
                    onChange={(value, name) => handleSelectChange(value, name)}
                  />
                  {errors.identityType && <p className="mt-1 text-sm text-red-500">{errors.identityType}</p>}
                </div>
                <div className="col-span-1">
                  <Label>
            ...
            <div className="flex items-center justify-end w-full gap-3 mt-6">
              ...
              <Button
                ...
                disabled={
                  ...
                  (formData.status === "CLIENT" && !formData.password.trim()) ||
                  (formData.status === "CLIENT" && formData.password !== formData.confirmPassword) ||
                  (formData.status === "CLIENT" && !formData.identityType.trim()) ||
                  (formData.status === "CLIENT" && !formData.identityNumber.trim()) ||
